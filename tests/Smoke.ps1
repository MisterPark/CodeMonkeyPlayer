...
try {
    ...
    $form.CreateControl()
    $player = Field 'axWindowsMediaPlayer1'
    ...
    Assert ($player.currentMedia.duration -gt 0) 'Media duration was not loaded'
    Assert ([int]$player.playState -eq 3) 'Media did not start playing'
    ...
    Pump 75
    Assert ([int]$player.playState -eq 2) 'Video click did not pause playback'
    InvokePrivate 'VideoClicked' @([int]1) | Out-Null
    Pump 75
    Assert ([int]$player.playState -eq 3) 'Video click did not resume playback'
    InvokePrivate 'VideoClicked' @([int]1) | Out-Null
    ...
    Assert ([int]$player.playState -eq 3) 'Double click changed playback state'
    InvokePrivate 'VideoDoubleClicked' @([int]1) | Out-Null
    InvokePrivate 'TogglePlayback' @() | Out-Null
    Pump 100
    Assert ([int]$player.playState -eq 2) 'Pause failed'
    (Field 'videoClickTimer').Stop()
    InvokePrivate 'VideoClicked' @([int]1) | Out-Null
    InvokePrivate 'VideoDoubleClicked' @([int]1) | Out-Null
    Pump 75
    Assert ([int]$player.playState -eq 2) 'Double click failed to preserve paused playback'
    InvokePrivate 'VideoDoubleClicked' @([int]1) | Out-Null
    InvokePrivate 'TogglePlayback' @() | Out-Null
    Pump 100
    Assert ([int]$player.playState -eq 3) 'Resume failed'
    ...
    Assert ($player.Ctlcontrols.currentPosition -ge 1) 'Seek failed'
    $seekControl = Field 'seek'
    $seekControl.CreateControl()
    $seekControl.Value = 9000
    $player.Ctlcontrols.currentPosition = 1.0
    [SeekMouseInput]::SendMessage($seekControl.Handle, 0x0101, [IntPtr]190, [IntPtr]0) | Out-Null
    Assert ([Math]::Abs($player.Ctlcontrols.currentPosition - 1.0) -lt 0.2) 'Frame key release rewound playback to the stale slider position'
    $seekControl.Value = 0
    ...
    Assert ([Math]::Abs($player.Ctlcontrols.currentPosition - 1.5) -lt 0.2) 'Mouse down did not seek immediately'
    ...
    $player.Ctlcontrols.currentPosition = 2.7
    Pump 1300
    ...
    Write-Output 'PASS: ActiveX initialization, media playback, pause/resume, seek, duplicate filtering, clear, remove.'
}
finally {
    $form.Dispose()
